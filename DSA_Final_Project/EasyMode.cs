...
namespace DSA_Final_Project
{
    public partial class EasyMode : Form
    {
        ...
        private WordPriorityQueue wordQueue;

        public EasyMode()
        {
            this.StartPosition = FormStartPosition.Manual;
            InitializeComponent();
            wordQueue = new WordPriorityQueue();
        }
        ...
        private void EasyMode_Load(object sender, EventArgs e)
        {
            ...
            gameEngine = new GameManager(7, 7, true);
            gameEngine.InitializeGame();
            InitializePictureGrid(7, 7);
            gameGraph = new Graph(7, 7, gameEngine.Grid);
            UpdateGrid();
        ...
        private void InitializeComponents()
        {
            ...
            this.Text = "Easy Mode - Word Search";
        ...
        private void RestartGameWithLargerGrid()
        {
            int newRows = pictureGrid.GetLength(0) + 1; // Increase grid size by 2 rows
            int newCols = pictureGrid.GetLength(1) + 1; // Increase grid size by 2 columns

            EasyMode newGame = new EasyMode();
            newGame.gameEngine = new GameManager(newRows, newCols, false); // Pass updated grid size
            newGame.gameEngine.InitializeGame();
            newGame.Show();
        ...
        private void PictureBox_Click(object sender, EventArgs e, int row, int col)
        {
            ...
            var currentWord = wordQueue.GetNextWord();

            foreach (var word in gameEngine.GetWordsToFind())
            {
                if (currentWord.StartsWith(selectedWord))
                {
                    List<(int, int)> selectedCellsList = selectedCells.ToList();
                    if (gameGraph.ValidateWordDFS(currentWord, selectedCellsList))
                    {
                        if (currentWord == selectedWord)
                        {
                            ...
                            gameEngine.GetWordsToFind().Remove(currentWord);
                            RemoveWordFromDisplay(currentWord);

                            wordQueue.RemoveNextWord();
                            ResetSelection();
                            ...
                            if (gameEngine.GetWordsToFind().Count == 0)
                            {
                                ...
                                DialogResult result = MessageBox.Show(
                                    $"Congratulations! You finished in {40 - timeLeft} seconds. Do you want to continue with a bigger grid?",
                                    "You Won!",
                                ...
                                {
                                    // Restart the game with increased grid size
                                    RestartGameWithLargerGrid();                                    
                                    
                                }
                                else
                                {
                                    winingForm winingForm = new winingForm($"Great Job! You completed the game.");
                                    winingForm.Show();
                                    this.Hide();
                                }
                            }
        ...
        {
            if (wordQueue.HasWords())
            {
                var nextWord = wordQueue.GetNextWord();
                wordLabel.Text = $"Selected Word: {nextWord}";
            }
        }

        private void RefreshGrid()
        {
            // Logic to refresh grid after each word is found
            InitializePictureGrid(gameEngine.Grid.GetLength(0), gameEngine.Grid.GetLength(1)); // You can adjust grid dimensions as needed

            // Update the pictures for the grid
            UpdateGrid();
        ...
        private void guna2PictureBox48_Click(object sender, EventArgs e)
        {
        ...
        private void DisplayHardcodedWords()
        {
            ...
            // Add words to the priority queue with priorities
            foreach (var word in wordsToFind)
            {
                wordQueue.AddWord(word, wordsToFind.IndexOf(word)); // Priority is the index
            }

            // Display the word with the highest priority (first word)
            var nextWord = wordQueue.GetNextWord();
